...
class RuledLinesRemoverSpec extends AnyFlatSpec with should.Matchers {
  val white = new Color(255, 255, 255).getRGB()
  val black = new Color(0, 0, 0).getRGB()

  it should "Can remove vertical/horizontal lines" in {
    ...
    for (x <- correctDelta until width - correctDelta)
      for (y <- correctDelta until height - correctDelta)
        assert(ruledLineRemovedImage.getRGB(x, y) === white)
  }

  it should "Can remove vertical lines" in {
    val fileName = "testdata/ruledLineRemover/test0002.png"
    // create test data
    if (!Paths.get(fileName).toFile.exists())
      createTestData(fileName)
  ...
  it should "Can remove vertical lines that thickness are equal to or more than 2" in {
    val fileName = "testdata/ruledLineRemover/test0002.png"
    // create test data
    if (!Paths.get(fileName).toFile.exists())
      createTestData(fileName)
    ...
    for (x <-
      ...
      for (y <- correctDelta until height - correctDelta)
        assert(ruledLineRemovedImage.getRGB(x, y) === black)
  }

  it should "Can remove vertical lines that thickness are equal to or more than 3" in {
    val fileName = "testdata/ruledLineRemover/test0002.png"
    // create test data
    if (!Paths.get(fileName).toFile.exists())
      createTestData(fileName)
  ...
  it should "Can remove horizontal lines" in {
    val fileName = "testdata/ruledLineRemover/test0002.png"
    // create test data
    if (!Paths.get(fileName).toFile.exists())
      createTestData(fileName)
  ...
  it should "Can remove horizontal lines that thickness are equal to or more than 3" in {
    val fileName = "testdata/ruledLineRemover/test0002.png"
    // create test data
    if (!Paths.get(fileName).toFile.exists())
      createTestData(fileName)
  ...
  it should "Can remove lines except charactors" in {
    val correctDelta = 2
    val bi = ImageIO.read(Paths.get("testdata/ruledLineRemover/test0001.png").toFile)
    val remover = new RuledLinesRemover(deltaX = 1, deltaY = 1, dotRatio = 40,
